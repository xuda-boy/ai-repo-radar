...
- Main business object: one repository recommendation and its single active feedback choice.
...
| Primary task | Record, cancel or replace one recommendation signal | Selection must be unmistakable |
| Data object | Repository + feedback action | Use a single-select action group |
...
|---|---|---|
| Feedback action | Selected control | `aria-pressed`, selected class, action label and visible “已选” badge |
| Feedback group | State block | Default, hover, submitting, selected and cancelled states |
...
- Selected: only the persisted action uses a solid primary surface, white content and “已选” badge.
...
- Preserve FastAPI, Jinja, HTMX, append-only feedback semantics and the existing color tokens.
