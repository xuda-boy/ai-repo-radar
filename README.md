...
- “更多此类 / 收藏 / 不相关 / 已了解”都是 UUID 事件，同一项目同一日报只保留一个当前选择：再次点击会取消，点击其他项会替换；反馈次日才改变推荐，撤回只追加抵消事件，原记录不会被删除。
...
Dashboard 点击反馈后会先写 `data/outbox/<UUID>.json`，并立即出现在“反馈”页。每个项目在一份日报中只有一个当前选择；再次点击已选按钮会取消，点击其他按钮会先抵消旧反馈再记录新选择。该页面会显示动作、提交时间、生效日期、同步状态和当前状态；所有取消与替换都追加次日生效的抵消事件，不删除原事件。只有数据目录位于带 `.ai-repo-radar-private` 标记的私人 Git 仓中，状态面板才会启用“立即同步到私有仓”；点击后会拉取、提交并推送。也可以使用等价的 CLI 重试：
